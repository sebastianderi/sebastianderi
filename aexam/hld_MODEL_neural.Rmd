...
Again, I will start by loading relevant packages. (I also am setting some other
global background options, that will control how the output gets saved for
future reuse and recreation of this document.)
...
```{r global_sets}
# set cache=TRUE for whole document
...
Tune parametes (iterations set to 100 by default)


```{r nnet_ex, cache = TRUE}
# set seed, so that statistics don't keep changing for every analysis
...
model_ex <- 
  train(form = grd_truth ~ . - stat_id,
        data = train_set_ex,
        method = "nnet",
        trace = FALSE) # don't print out after 
...
```{r nnet_ex_conf, dependson='nnet_ex'}
# make predictions
preds_ex <-
